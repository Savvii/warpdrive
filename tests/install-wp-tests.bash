...
if [[ $WP_VERSION =~ ([0-9]+\.[0-9]+)\.X ]]; then
  WP_VERSION_BASE="${BASH_REMATCH[1]}"
  PATCH_NUMBER=1
  for ((;; ((PATCH_NUMBER=PATCH_NUMBER+1)))); do
    ...
    if [ "${WP_TEST_RESULT}" -gt '200' ]; then
      # Substract 1 from PATCH_NUMBER as the last failed
      ((PATCH_NUMBER=PATCH_NUMBER-1))
      break
    fi
  done
  # If PATCH_NUMBER is 0, the version should be formatted as X.Y instead of X.Y.X
  if [ "${PATCH_NUMBER}" -eq '0' ]; then
    WP_VERSION="${WP_VERSION_BASE}"
  else
    WP_VERSION="${WP_VERSION_BASE}.${PATCH_NUMBER}"
  fi
